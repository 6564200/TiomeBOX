...
def internet_connection():
    	try:
		socket.gethostbyaddr('185.188.182.233')
...
def send_to_WEB(s={}):
    ...
    url_api = "http://kazmaz.pythonanywhere.com/max/default/reglog.html?cam=%s&t=%s&w=%s&p=%s&r=%s&seting=%s&ch=%s" % (s['cam'], s['temp'], s['water'], s['reg_on'], s['reg_on'], s['setcapture'], s['chec'])
    print url_api
    while 1:
            try:
